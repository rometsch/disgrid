  ...
  double *x1, *x2, *x3;
    ...
    for (i = beg; i <= end; i++) p->a2[i] = g_gamma*p->v[i][PRS]/p->v[i][RHO];
     ...
     if (g_dir == IDIR) {
       ...
       for (i = beg; i <= end; i++) p->a2[i] = g_isoSoundSpeed*g_isoSoundSpeed/x1[i];

     }else if (g_dir == JDIR){

       x2 = (pos == FACE_CENTER ? grid->xr[JDIR] : grid->x[JDIR]);
       for (j = beg; j <= end; j++) p->a2[j] = g_isoSoundSpeed*g_isoSoundSpeed/x1[i];

     }else if (g_dir == KDIR){

       x3 = (pos == FACE_CENTER ? grid->xr[KDIR] : grid->x[KDIR]);
       for (k = beg; k <= end; k++) p->a2[k] = g_isoSoundSpeed*g_isoSoundSpeed/x1[i];
