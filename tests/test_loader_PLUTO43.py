...
class TestPLUTO43Loader(unittest.TestCase):
    ...
    def test_can_calc_scale_height(self):
        if self.d.loader.eos == 'IDEAL':
            ...
            #r[36] = 5.194 au
            self.assertTrue(
                close(data.data[36][0][0].to('au'), 0.05 * 5.2 * u.au))
    ...
    def test_gasvrad(self):
        ...
        self.assertEqual(vrad.data.shape,
                         (len(vrad.grid.get_coordinates("r")),
                          len(vrad.grid.get_coordinates("theta")),
    ...
    def test_scalar_mass(self):
        ...
        self.assertTrue(
            close(mass.data[2].decompose().cgs, 9.5536e-01 * 2e+33 * u.g))
        self.assertTrue(
            close(
                mass.time[2].decompose().cgs,
...
def close(x, y):
    if np.abs(x - y) < np.abs(1e-2 * x):
        ...
        try:
            ...
            print('failed to match: %.3le to %.3le' % (x, y))
        return False
