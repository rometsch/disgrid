...
vars_2d = {
    ...
    ,"pressure" : {
        ...
        "shorthand"  : "prs",
        "numvar" : NOT_FOUND,
        "unitpowers" : {"mass" : 1, "time" : -2, "length" : 0}
        }
    ,"vrad" : {
        "pattern" : "vx1.{}.dbl",
        "shorthand"  : "vx1",
...
class Loader(interface.Interface):
    ...
    def get_units(self):
        self.units = PUnits.loadUnits(self.data_dir, dimensions=self.dimensions)
...
class FieldLoader2d(interface.FieldLoader):
    ...
    def load_grid(self, n=0):
        """Returns a simdata.grid structure in relevant coordinates."""
        return PGrid.loadGrid(self.loader.data_dir, length_unit=self.loader.units["length"], angle_unit=u.radian)
