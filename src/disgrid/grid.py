...
class Grid:
    def __init__(self, active_interfaces=[], names=[]):
        ...
        for key in active_interfaces:
            if key not in names:
                raise ValueError(
                    "Can't declare active interface for dim '{}' which is not in '{}'"
                    .format(key, names))

    def get_coordinates(self, key):
        if not key in self.names:
            raise KeyError(
                "Dimension '{}' not defined for this grid.".format(key))
    ...
    def __init__(self,
        ...
        for n in range(1, 4):
            ...
            if all([xi is None, xc is None, dx is None]):
                ...
                raise ValueError(
                    "Only cell sizes given for dim nr. {}".format(n))
            elif xi is not None:
                if xc is None:
                ...
                if xi is None and not dx is None:
                    ...
                    raise ValueError(
                        "Can't calculate cell interfaces for dim nr. {}. Cell sizes dx not given"
                        .format(n))
